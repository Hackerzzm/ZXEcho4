...
public class MyEngineEventHandler {
  ...
  final IRtcEngineEventHandler mRtcEventHandler = new IRtcEngineEventHandler() {
    ...
    @Override public void onUserMuteVideo(int uid, boolean muted) {
    ...
    @Override public void onAudioVolumeIndication(AudioVolumeInfo[] speakerInfos, int totalVolume) {
    ...
    @Override public void onStreamMessage(int uid, int streamId, byte[] data) {
      log.debug(
          "onStreamMessage " + (uid & 0xFFFFFFFFL) + " " + streamId + " " + Arrays.toString(data));
    ...
    public void onStreamMessageError(int uid, int streamId, int error, int missed, int cached) {
      ...
      while (it.hasNext()) {
        AGEventHandler handler = it.next();
        handler.onExtraCallback(AGEventHandler.EVENT_TYPE_ON_AGORA_MEDIA_ERROR, error,
            "on stream msg error " + (uid & 0xFFFFFFFFL) + " " + missed + " " + cached);
      }
    }
    ...
    @Override public void onJoinChannelSuccess(String channel, int uid, int elapsed) {
      log.debug("onJoinChannelSuccess "
          + channel
          + " "
          + uid
          + " "
