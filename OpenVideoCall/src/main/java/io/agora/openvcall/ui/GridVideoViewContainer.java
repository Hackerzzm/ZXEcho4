...
public class GridVideoViewContainer extends RecyclerView {
  ...
  private boolean initAdapter(Activity activity, int localUid, HashMap<Integer, SurfaceView> uids) {
    if (mGridVideoViewContainerAdapter == null) {
      mGridVideoViewContainerAdapter =
          new GridVideoViewContainerAdapter(activity, localUid, uids, mEventListener);
  ...
  public void initViewContainer(Activity activity, int localUid,
    ...
    if (count <= 2) { // only local full view or or with one peer
      this.setLayoutManager(
          new LinearLayoutManager(activity.getApplicationContext(), RecyclerView.VERTICAL, false));
    } else if (count > 2 && count <= 4) {
      this.setLayoutManager(
          new GridLayoutManager(activity.getApplicationContext(), 2, RecyclerView.VERTICAL, false));
    }

    mGridVideoViewContainerAdapter.notifyDataSetChanged();
  }

  public void notifyUiChanged(HashMap<Integer, SurfaceView> uids, int localUid,
