...
public class LiveRoomActivity extends BaseActivity implements AGEventHandler {
  ...
  @Override protected void initUIandEvent() {
    ...
    if (isBroadcaster(cRole)) {
      ...
      mGridVideoViewContainer.initViewContainer(getApplicationContext(), 0,
          mUidsList); // first is now full view
  ...
  private void broadcasterUI(ImageView button1, ImageView button2, ImageView button3) {
    ...
    button3.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View v) {
        ...
        button.setTag(flag);
        if (flag) {
          button.setColorFilter(getResources().getColor(R.color.agora_blue),
  ...
  private void doSwitchToBroadcaster(boolean broadcaster) {
    ...
    log.debug("doSwitchToBroadcaster "
        + currentHostCount
        + " "
        + (uid & 0XFFFFFFFFL)
        + " "
    ...
    runOnUiThread(new Runnable() {
      @Override public void run() {
        ...
        if (mViewType == VIEW_TYPE_DEFAULT) {
          ...
          log.debug("doRenderRemoteUi VIEW_TYPE_SMALL" + " " + (uid & 0xFFFFFFFFL) + " " + (bigBgUid
              & 0xFFFFFFFFL));
    ...
    runOnUiThread(new Runnable() {
      @Override public void run() {
        ...
        if (mUidsList.containsKey(uid)) {
          log.debug(
              "already added to UI, ignore it " + (uid & 0xFFFFFFFFL) + " " + mUidsList.get(uid));
          return;
        }

        final boolean isBroadcaster = isBroadcaster();
        log.debug(
            "onJoinChannelSuccess " + channel + " " + uid + " " + elapsed + " " + isBroadcaster);
    ...
    new Handler().postDelayed(new Runnable() {
      @Override public void run() {
        ...
        for (HashMap.Entry<Integer, SurfaceView> pair : mUidsList.entrySet()) {
          log.debug("requestRemoteStreamType " + currentHostCount + " local " + (config().mUid
              & 0xFFFFFFFFL) + " " + (pair.getKey() & 0xFFFFFFFFL) + " " + pair.getValue()
              .getHeight() + " " + pair.getValue().getWidth());
          if (pair.getKey() != config().mUid && (highest == null
              || highest.getValue().getHeight() < pair.getValue().getHeight())) {
            if (highest != null) {
            ...
            highest = pair;
          } else if (pair.getKey() != config().mUid && (highest != null
              && highest.getValue().getHeight() >= pair.getValue().getHeight())) {
  ...
  private void switchToDefaultVideoView() {
    ...
    for (int i = 0; i < sizeLimit; i++) {
      ...
      if (config().mUid != uid) {
        ...
        log.debug("setRemoteVideoStreamType VIDEO_STREAM_HIGH " + mUidsList.size() + " " + (uid
            & 0xFFFFFFFFL));
      }
    }
  }
  ...
  private void bindToSmallVideoView(int exceptUid) {
    ...
    if (mSmallVideoViewAdapter == null) {
      ...
      mSmallVideoViewAdapter =
          new SmallVideoViewAdapter(this, exceptUid, mUidsList, new VideoViewEventListener() {
            @Override public void onItemDoubleClick(View v, Object item) {
              switchToDefaultVideoView();
            }
          });
