...
public class GridVideoViewContainerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
  ...
  public GridVideoViewContainerAdapter(Context context, int localUid,
      HashMap<Integer, SurfaceView> uids, VideoViewEventListener listener) {
    mContext = context;
  ...
  public void init(HashMap<Integer, SurfaceView> uids, int localUid, boolean force) {
    for (HashMap.Entry<Integer, SurfaceView> entry : uids.entrySet()) {
      if (entry.getKey() == 0 || entry.getKey() == mLocalUid) {
        ...
        for (VideoStatusData status : mUsers) {
          if ((status.mUid == entry.getKey() && status.mUid == 0)
              || status.mUid == mLocalUid) { // first time
        ...
        if (!found) {
          mUsers.add(0,
              new VideoStatusData(mLocalUid, entry.getValue(), VideoStatusData.DEFAULT_STATUS,
                  VideoStatusData.DEFAULT_VOLUME));
        }
      } else {
        boolean found = false;
        for (VideoStatusData status : mUsers) {
        ...
        if (!found) {
          mUsers.add(
              new VideoStatusData(entry.getKey(), entry.getValue(), VideoStatusData.DEFAULT_STATUS,
                  VideoStatusData.DEFAULT_VOLUME));
        }
      }
    }
    ...
    while (it.hasNext()) {
      ...
      if (uids.get(status.mUid) == null) {
        log.warn("after_changed remove not exited members "
            + (status.mUid & 0xFFFFFFFFL)
            + " "
            + status.mView);
        it.remove();
      }
  ...
  @Override public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
    ...
    if (holderView.getChildCount() == 0) {
      ...
      holderView.addView(target, new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
