...
import zzm.zxecho.openlive.model.AGEventHandler;
import zzm.zxecho.openlive.model.ConstantApp;
import zzm.zxecho.openlive.model.VideoStatusData;
...
public class LiveRoomActivity extends BaseActivity implements AGEventHandler {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(zzm.zxecho.openlive.R.layout.activity_live_room);
  }
  ...
  @Override protected void initUIandEvent() {
    ...
    mGridVideoViewContainer = (GridVideoViewContainer) findViewById(zzm.zxecho.openlive.R.id.grid_video_view_container);
    ...
    ImageView button1 = (ImageView) findViewById(zzm.zxecho.openlive.R.id.btn_1);
    ImageView button2 = (ImageView) findViewById(zzm.zxecho.openlive.R.id.btn_2);
    ImageView button3 = (ImageView) findViewById(zzm.zxecho.openlive.R.id.btn_3);
    ...
    TextView textRoomName = (TextView) findViewById(zzm.zxecho.openlive.R.id.room_name);
  ...
  private void broadcasterUI(ImageView button1, ImageView button2, ImageView button3) {
    ...
    button1.setColorFilter(getResources().getColor(zzm.zxecho.openlive.R.color.agora_blue), PorterDuff.Mode.MULTIPLY);
    ...
    button3.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View v) {
        ...
        button.setTag(flag);
        if (flag) {
          button.setColorFilter(getResources().getColor(zzm.zxecho.openlive.R.color.agora_blue), PorterDuff.Mode.MULTIPLY);
        } else {
  ...
  private void doShowButtons(boolean hide) {
    View topArea = findViewById(zzm.zxecho.openlive.R.id.top_area);
    topArea.setVisibility(hide ? View.INVISIBLE : View.VISIBLE);

    View button1 = findViewById(zzm.zxecho.openlive.R.id.btn_1);
    button1.setVisibility(hide ? View.INVISIBLE : View.VISIBLE);

    View button2 = findViewById(zzm.zxecho.openlive.R.id.btn_2);
    View button3 = findViewById(zzm.zxecho.openlive.R.id.btn_3);
    if (isBroadcaster()) {
  ...
  @Override public void onJoinChannelSuccess(final String channel, final int uid, final int elapsed) {
    runOnUiThread(new Runnable() {
      @Override public void run() {
        if (isFinishing()) {
          return;
    ...
    new Handler().postDelayed(new Runnable() {
      @Override public void run() {
        ...
        for (HashMap.Entry<Integer, SurfaceView> pair : mUidsList.entrySet()) {
          log.debug("requestRemoteStreamType "
              + currentHostCount
              + " local "
          ...
          if (pair.getKey() != config().mUid && (highest == null || highest.getValue().getHeight() < pair.getValue().getHeight())) {
            ...
            log.debug("setRemoteVideoStreamType VIDEO_STREAM_LOW " + currentHostCount + " " + (pair.getKey() & 0xFFFFFFFFL) + " " + pair.getValue()
        ...
        if (highest != null && highest.getKey() != 0) {
          ...
          log.debug(
              "setRemoteVideoStreamType VIDEO_STREAM_HIGH " + currentHostCount + " " + (highest.getKey() & 0xFFFFFFFFL) + " " + highest.getValue()
  ...
  private void switchToDefaultVideoView() {
    if (mSmallVideoViewDock != null) mSmallVideoViewDock.setVisibility(View.GONE);
  ...
  private void bindToSmallVideoView(int exceptUid) {
    if (mSmallVideoViewDock == null) {
      ViewStub stub = (ViewStub) findViewById(zzm.zxecho.openlive.R.id.small_video_view_dock);
      mSmallVideoViewDock = (RelativeLayout) stub.inflate();
    }

    RecyclerView recycler = (RecyclerView) findViewById(zzm.zxecho.openlive.R.id.small_video_view_container);
